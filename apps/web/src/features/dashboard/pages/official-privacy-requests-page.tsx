    ...
    <main className="min-h-full bg-[#f7f8fc] p-4 md:p-6 lg:p-8">
      <section className="rounded-2xl border border-[#dfe7f5] bg-white p-5">
        ...
        <div className="mt-5 flex flex-col gap-3 border-t border-[#e7edf8] pt-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as StatusFilter)} className="h-10 rounded-lg border border-[#dfe7f5] bg-white px-3 text-xs font-bold text-brand-navy">
          ...
          {visible.map((request) => (
            <article key={request.id} className="rounded-2xl border border-[#dfe7f5] bg-white p-5">
              ...
              <div className="mt-4 rounded-xl bg-[#f8fafc] p-3">
                ...
                {request.staff_note ? <p className="mt-2 border-t border-[#dfe7f5] pt-2 text-xs font-semibold text-brand-navy">Official note: {request.staff_note}</p> : null}
              </div>
            ...
            <div className="divide-y divide-[#e7edf8]">
              {accessRows.map((row) => (
                <div key={row.id} className="grid gap-2 px-4 py-4 text-xs md:grid-cols-[1.3fr_1.3fr_1fr_1fr] md:items-center md:gap-3">
                  <div><span className="font-black text-brand-navy">{row.actor?.full_name || "System"}</span><span className="mt-0.5 block truncate text-[11px] font-semibold text-[#68739c]">{row.actor?.email || "No user actor"}</span></div>
                  <div><span className="font-black text-brand-navy">{row.subject?.full_name || "No subject"}</span><span className="mt-0.5 block truncate text-[11px] font-semibold text-[#68739c]">{row.subject?.email || "—"}</span></div>
