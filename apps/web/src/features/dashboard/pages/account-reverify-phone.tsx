...
export default function AccountReverifyPhonePage() {
  usePageTitle("Verify mobile number")
  const navigate = useNavigate()
  const location = useLocation()
  const { user, setAuthenticatedUser, refreshUser } = useAuthSession()
  const draftFromState =
    (location.state as { phone?: string } | null)?.phone?.trim() || ""

  const [step, setStep] = useState<"enter" | "otp">("enter")
  const [phoneLocal, setPhoneLocal] = useState(() =>
    localFromAny(draftFromState || user?.phone_number || ""),
  )
  ...
  async function verifyOtp() {
    ...
    try {
      ...
      let pendingEmail = ""
      try {
        pendingEmail = sessionStorage.getItem("eboses-pending-email-change") || ""
        sessionStorage.removeItem("eboses-pending-email-change")
      } catch {
        /* ignore */
      }
      if (pendingEmail) {
      ...
      <div className="mx-auto w-full max-w-[600px] flex-1 px-5 pb-10 pt-4 md:px-0 md:pt-6">
        ...
        {step === "enter" ? (
          <>
            ...
            {/* Sign-up style: flag +63 + floating-height input */}
