...
def deliver_notification_task(notification_id):
    ...
    logger.info("deliver_notification_task START notification_id=%s", notification_id)
    notification = Notification.objects.select_related("recipient", "emergency", "concern").filter(
        pk=notification_id
    ).first()
    if not notification:
        logger.info("deliver_notification_task SKIP notification_id=%s not_found", notification_id)
