...
function Sheet({ children, open: controlledOpen, onOpenChange, lockScroll = true, }) {
...
const sidePanelClass = {
    bottom: "inset-x-0 bottom-0 top-auto max-h-[85vh] w-full rounded-t-2xl pb-[env(safe-area-inset-bottom)]",
...
function SheetHeader({ className, children, ...props }) {
    const { setOpen } = useSheet();
    const title = React.Children.toArray(children).find((child) => React.isValidElement(child) && child.type === SheetTitle);
    return (<div className={cn("flex shrink-0 items-center justify-between border-b border-border/50 px-4 py-3 sm:px-6 sm:py-4", className)} {...props}>
      ...
      <button type="button" onClick={() => setOpen(false)} className="ml-4 flex size-7 shrink-0 items-center justify-center rounded-lg text-black transition-colors hover:bg-muted hover:text-black sm:size-8" aria-label="Close">
        <XIcon className="size-4 text-black"/>
      </button>
    </div>);
}
function SheetBody({ className, ...props }) {
    return (<div className={cn("flex-1 px-4 pb-4 pt-4 space-y-4", "sm:px-6 sm:py-5 sm:space-y-5", className)} {...props}/>);
}
...
export { Sheet, SheetTrigger, SheetContent, SheetHeader, SheetBody, SheetFooter, SheetTitle, SheetClose, useSheet, };
