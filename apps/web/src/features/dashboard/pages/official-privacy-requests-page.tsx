...
export default function OfficialPrivacyRequestsPage() {
  ...
  return (
    <main className="min-h-full bg-canvas p-4 md:p-6 lg:p-8">
      <section className="rounded-2xl border border-line-tint bg-white p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-[12px] font-black uppercase tracking-wide text-brand-orange">Configuration · Privacy</p>
            <h1 className="mt-1 text-2xl font-black text-brand-navy">Resident privacy requests</h1>
        ...
        <div className="mt-5 flex flex-col gap-3 border-t border-line-tint pt-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap gap-2" aria-label="Request type filter">
            {(["all", "data_export", "deletion"] as RequestTypeFilter[]).map((value) => (
              <button key={value} type="button" onClick={() => setTypeFilter(value)} className={cn("rounded-md border px-3 py-2 text-xs font-black", typeFilter === value ? "border-brand-blue bg-brand-blue text-white" : "border-line-tint bg-canvas text-navy-muted")}>{value === "all" ? "All requests" : value === "data_export" ? "Data exports" : "Deletion"}</button>
            ))}
          </div>
        ...
        <section className="mt-4 grid gap-4 xl:grid-cols-2">
          {visible.map((request) => (
            <article key={request.id} className="rounded-2xl border border-line-tint bg-white p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <h2 className="truncate font-black text-brand-navy">{personName(request)}</h2>
              ...
              <div className="mt-4 rounded-xl bg-canvas p-3">
                <p className="text-[11px] font-black uppercase text-subtle-foreground">{request.type === "data_export" ? "Data export" : "Account deletion"}</p>
              ...
              {!["completed", "rejected"].includes(request.status) ? (
                <div className="mt-4">
                  <label htmlFor={`privacy-note-${request.id}`} className="text-[11px] font-black uppercase text-subtle-foreground">Official decision note</label>
          ...
          {visible.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-line-tint bg-white p-10 text-center xl:col-span-2">
              ...
              <h2 className="mt-3 font-black text-brand-navy">No privacy requests in this view</h2>
      ...
      <section className="mt-6 rounded-2xl border border-line-tint bg-white p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex items-center gap-2 text-brand-blue"><EyeIcon className="size-5" /><p className="text-[12px] font-black uppercase tracking-wide">Sensitive access review</p></div>
            <h2 className="mt-1 text-xl font-black text-brand-navy">Protected information access</h2>
        ...
        <div className="mt-5 flex flex-col gap-3 border-t border-line-tint pt-4 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-wrap gap-2">
            {(["all", "media", "export"] as const).map((value) => <button key={value} type="button" onClick={() => setAccessKind(value)} className={cn("rounded-md border px-3 py-2 text-xs font-black", accessKind === value ? "border-brand-blue bg-brand-blue text-white" : "border-line-tint bg-canvas text-navy-muted")}>{value === "all" ? "All access" : value === "media" ? "Protected media" : "Data exports"}</button>)}
          </div>
        ...
        {!accessLoading && !accessError ? (
          <div className="mt-4 overflow-hidden rounded-xl border border-line-tint">
            <div className="hidden grid-cols-[1.3fr_1.3fr_1fr_1fr] gap-3 bg-canvas px-4 py-3 text-[10px] font-black uppercase text-subtle-foreground md:grid"><span>Accessed by</span><span>Resident / subject</span><span>Protected resource</span><span>When</span></div>
            <div className="divide-y divide-line-tint">
              {accessRows.map((row) => (
                <div key={row.id} className="grid gap-2 px-4 py-4 text-xs md:grid-cols-[1.3fr_1.3fr_1fr_1fr] md:items-center md:gap-3">
                  <div><span className="font-black text-brand-navy">{row.actor?.full_name || "System"}</span><span className="mt-0.5 block truncate text-[11px] font-semibold text-subtle-foreground">{row.actor?.email || "No user actor"}</span></div>
                  <div><span className="font-black text-brand-navy">{row.subject?.full_name || "No subject"}</span><span className="mt-0.5 block truncate text-[11px] font-semibold text-subtle-foreground">{row.subject?.email || "—"}</span></div>
                  <div><span className="rounded-md bg-tint px-2 py-1 font-black text-brand-blue">{row.resource_type.replace(/_/g, " ")}</span><span className="mt-1 block text-[10px] font-semibold text-subtle-foreground">Record {row.resource_id || "—"}</span></div>
