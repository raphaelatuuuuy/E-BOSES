...
export function statusClass(status: EmergencyStatus) {
  const critical: EmergencyStatus[] = [
    "submitted",
    "routing",
    "escalation_required",
    "transfer_required",
  ...
  const waiting: EmergencyStatus[] = [
    "routed",
    "awaiting_acknowledgment",
    "acknowledged",
    "backup_requested",
  ]
  const moving: EmergencyStatus[] = [
    "en_route",
    "nearby",
    "backup_assigned",
    "in_progress",
  ]
  const done: EmergencyStatus[] = [
    "arrived",
    "resolved",
    "closed",
    "resident_safe",
  ...
  if (critical.includes(status))
    return "border-severity-critical/40 bg-severity-critical-surface text-severity-critical-ink"
  if (waiting.includes(status))
    return "border-severity-moderate/40 bg-severity-moderate-surface text-severity-moderate-ink"
  if (moving.includes(status))
    return "border-status-active/40 bg-status-active-surface text-status-active-ink"
  if (done.includes(status))
    return "border-status-closed/40 bg-status-closed-surface text-status-closed-ink"
...
export function responderStatusLabel(
  responder?: PublicUser | null,
  viewerId?: number | null
) {
  if (!responder) return ""
  return responder.id === viewerId
    ? "You are responding"
    : `${responderName(responder)} is responding`
}

export const RESPONDER_ASSIGNMENT_STATUSES = new Set([
  "assigned",
  "acknowledged",
  "en_route",
  "nearby",
...
export function emergencyResponderAssignments(alert: EmergencyAlert) {
  ...
  const allowed = settled
    ...
    : new Set(
        [...RESPONDER_ASSIGNMENT_STATUSES].filter(
          (status) => status !== "resolved"
        )
      )
  const assignments = [
    ...(alert.assignments ?? []),
    ...(alert.current_assignment ? [alert.current_assignment] : []),
  ].filter((assignment) => allowed.has(assignment.status))
...
export function routeSummary(route: EmergencyAlert["route"]) {
  if (!route || route.status === "unavailable") return "Route pending"
  const distance =
  ...
  return [distance, eta].filter(Boolean).join(" · ") || "Route pending"
}

/** A settled route should not display a live ETA. */
export function historicalRouteSummary(route: EmergencyAlert["route"]) {
  if (!route || route.status === "unavailable") return "Route history recorded"
  return (
    route.summary?.trim() ||
    routeSummary(route).replace(/\s*·\s*\d+ min ETA$/, "")
  )
}

export function emergencyRouteSummary(alert: EmergencyAlert) {
  return routeSummary(alert.current_assignment?.route ?? alert.route)
}
...
export function hasAutoRoute(alert: EmergencyAlert) {
  return (alert.status_events ?? []).some((event) =>
    event.note.toLowerCase().includes("auto-routed")
  )
}
 ...
 */
export const EMERGENCY_FILTERS = ["All", "Active", "Resolved"] as const
