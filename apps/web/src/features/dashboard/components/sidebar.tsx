 ...
 * treatment, badge language and account block. Residents, officials and
 * responders all use the same light surface; only item data differs.
 */

type Tone = "light" | "dark"

function workspaceTone(): Tone {
  return "light"
}

/**
 * Resident-only SOS entry pinned into the rail — opens the same SOS dialog
 * the floating SOSButton does, via the `eboses:open-sos` window event. Blinks
 * whenever an emergency of the resident's own is live.
 */
function SosRailRow({ onHover }: { onHover?: () => void }) {
  ...
  return (
    <button
      ...
      onBlur={() =>
        window.dispatchEvent(new CustomEvent("eboses:sos-veil-disarm"))
      }
    ...
    >
      <span
        className={cn(
          "material-symbols-outlined shrink-0 text-[26px] leading-none text-sos transition-transform duration-200 ease-out select-none hover:scale-[1.16]",
  ...
  return (
    <Link
      ...
      className={cn(
        "group relative",
        iconOnly
          ? "justify-center rounded-none px-0 hover:!bg-transparent"
          : "",
    ...
    >
      {iconOnly && tone === "light" ? (
        plainAlarmIcon ? (
          <Icon
            className={cn(
              "size-[26px] shrink-0 text-sos transition-transform duration-200 ease-out group-hover:scale-[1.16]",
      ...
      {badge && badge > 0 && !iconOnly ? (
        <span
          className={cn(
            ...
            urgent
              ? "bg-white font-bold text-sos"
              : "bg-sos font-bold text-white",
            urgent && "flex items-center gap-1",
            iconOnly && "absolute top-1/2 right-8 -translate-y-1/2"
          )}
        >
          {urgent ? (
            <span
              aria-hidden
              className="size-1.5 animate-pulse rounded-full bg-sos"
            />
          ) : null}
          {badge > 99 ? "99+" : badge}
        </span>
      ...
      {iconOnly ? (
        <span
          className={cn(
            "pointer-events-none absolute top-1/2 left-[calc(100%+10px)] z-[1100] w-52 translate-x-1 -translate-y-1/2 rounded-xl px-6 pt-5 pb-5 text-center opacity-0 shadow-[0_14px_35px_rgba(15,23,42,0.16)] ring-1 transition-[opacity,transform] duration-150 ease-out group-hover:translate-x-0 group-hover:opacity-100 before:absolute before:top-1/2 before:left-[-6px] before:size-3 before:-translate-y-1/2 before:rotate-45 before:border-b before:border-l",
            tooltipSurface
          )}
        >
          ...
          <span className="mt-3.5 block text-[14px] leading-tight font-bold">
            {item.label}
          </span>
          <span
            className={cn(
              "mt-2 block text-[12px] leading-[1.45]",
...
export function Sidebar() {
  ...
  return (
    ...
    >
      <nav
        className={cn(
          "flex min-h-0 flex-1 [scrollbar-width:thin] flex-col overscroll-contain px-3 pt-6",
          "overflow-visible",
          (isResident || isOfficialRole || isResponderRole) &&
            "justify-center pt-0"
        )}
      >
        <ul className="flex flex-col gap-2.5">
          ...
          {navItems.map((item) => {
            ...
            // The Concerns tab can only show live emergencies when the
            // official can dispatch them — without the capability the queue
            // endpoint 403s and "Urgent cases" is always empty, so the
            // summary's community-wide count must not blink the row for
            // emergencies that account cannot even see.
            const canSeeEmergencyQueue = hasCapability(
              user?.capabilities,
      ...
      <div className="relative shrink-0 px-3 pt-2 pb-4">
        <div>
          {open ? (
            <ul
              className={cn(
                "absolute bottom-6 left-[calc(100%+6px)] z-[1100] flex w-52 flex-col gap-0.5 rounded-xl p-3 shadow-[0_14px_35px_rgba(15,23,42,0.16)] ring-1 before:absolute before:bottom-4 before:left-[-6px] before:size-3 before:rotate-45 before:border-b before:border-l",
                accountSurfaceClass
              )}
            >
              <li>
          ...
          >
            ...
            <span className={cn("flex min-w-0 flex-1 flex-col", "hidden")}>
              <span
                className={cn(
                  "truncate text-[12.5px] leading-tight font-bold",
              ...
              <span
                className={cn(
                  "truncate text-[10.5px] leading-tight font-semibold",
                  tone === "dark" ? "text-nav-muted" : "text-neutral-500"
                )}
              >
                {rolePosition}
