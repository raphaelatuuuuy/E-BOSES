...
export function isPublicPageModule(id) {
...
export function nativeBuild() {
    ...
    return {
        ...
        enforce: "pre",
        transform(code, id) {
            if (!id.replaceAll("\\", "/").endsWith("/packages/ui/src/styles/globals.css"))
                return;
        ...
        generateBundle(_, bundle) {
            for (const output of Object.values(bundle)) {
                ...
                for (const id of Object.keys(output.modules)) {
                    if (isPublicPageModule(id))
                        this.error(`Public page included in native build: ${id}`);
                }
            }
        },
        closeBundle() {
