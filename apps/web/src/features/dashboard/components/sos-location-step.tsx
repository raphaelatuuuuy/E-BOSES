...
async function reverseStreet(lat: number, lng: number): Promise<{ primary: string; full: string }> {
  try {
    const matched = await reverseGeocodeToMarikinaStreet(lat, lng)
    if (matched.ok && matched.street) {
      const primary = matched.houseNumber ? `${matched.houseNumber} ${matched.street}` : matched.street
  ...
  try {
    ...
    const res = await fetch(url, {
      headers: { Accept: "application/json", "User-Agent": "E-Boses/1.0 (sos-location)" },
    })
    ...
    const primary = curated || road || (data.display_name ?? "").split(",")[0]?.trim() || "Pinned location"
    if (/^lat\b/i.test(primary) || primary.toLowerCase() === "marikina heights") {
  ...
  return (
    <div className={cn("flex min-h-0 flex-1 flex-col gap-3", className)}>
      <div className="flex items-center gap-3 rounded-xl border border-neutral-200 bg-white px-3.5 py-3">
        <img src="/contents/map-pin-gps.png" alt="" className="size-6 shrink-0 object-contain" aria-hidden />
        <div className="min-w-0 flex-1">
          <p className="truncate text-[15px] font-medium text-neutral-900">
            {gpsBusy || geocoding ? "Finding street…" : usable ? primary : "Move pin to a street"}
          </p>
          <p className="text-[12px] text-neutral-500">Drag the map to adjust · pin stays centered</p>
        </div>
        {(gpsBusy || geocoding) && <Loader2Icon className="size-4 shrink-0 animate-spin text-neutral-400" />}
      </div>

      <div className="relative min-h-[220px] flex-1 overflow-hidden rounded-xl border border-neutral-200 bg-[#e8eef5]">
        <div ref={containerRef} className="sos-loc-map absolute inset-0 z-0 h-full w-full" />
        {/* Center pin overlay */}
        <div className="pointer-events-none absolute left-1/2 top-1/2 z-[500] h-0 w-0" aria-hidden>
          <span
      ...
      {error ? <p className="text-[13px] font-medium text-amber-700">{error}</p> : null}
    </div>
  )
}

export function isSosLocationReady(value: SosLocationValue | null) {
  if (!value) return false
  const p = (value.addressPrimary || value.address || "").trim()
  if (!p || p === "Move pin to a street") return false
  if (/^lat\b/i.test(p)) return false
