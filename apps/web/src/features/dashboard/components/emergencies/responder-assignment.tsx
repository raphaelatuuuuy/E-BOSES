...
import { distanceKm, estimatedEtaMinutes, formatDistanceKm, responderName, unitLabel } from "./lib"
...
export function ResponderAssignment({
  ...
  /** Map panel keeps the list short; ops shows every on-duty responder. */
  ...
  const sorted = useMemo(() => {
    ...
    return copy.sort((a, b) => {
      return (distanceKm(alert.latitude, alert.longitude, a.latitude, a.longitude) ?? 999) -
        (distanceKm(alert.latitude, alert.longitude, b.latitude, b.longitude) ?? 999)
    })
  }, [alert, responders])
    ...
    <div className="space-y-4">
      <div className="grid gap-2">
        {alert?.assignments.length ? alert.assignments.map((assignment) => {
          ...
          <p className="rounded-control bg-card-raised px-3 py-2 text-xs font-semibold text-subtle-foreground">No response team has been assigned.</p>
        )}
      </div>
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-subtle-foreground">
          On-duty responders, nearest first. Distances are straight-line, so travel time is an estimate.
        </p>
      </div>

      <div className="grid gap-2">
        {visible.length === 0 ? (
          <p className="rounded-control bg-card-raised px-3 py-2 text-xs font-semibold text-subtle-foreground">No on-duty responders with live location yet.</p>
        ) : visible.map((responder) => {
          const km = alert ? distanceKm(alert.latitude, alert.longitude, responder.latitude, responder.longitude) : null
            ...
            <div key={responder.id} className={cn("rounded-control border p-3", assigned ? "border-brand-orange bg-brand-orange-soft" : "border-card-line bg-card")}>
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-brand-navy">{responder.full_name}</p>
                  {/* Distance and travel time are what decide who goes, so they
                      sit on the row. The "On duty" pill that used to hang below
                      said nothing — this list only ever contains on-duty
                      responders. */}
                ...
                >
                  {busyId === responder.id ? "Adding" : assigned ? "Assigned" : "Dispatch"}
                </Button>
              </div>
