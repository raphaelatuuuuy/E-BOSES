...
const DEFAULT_CLASSES = {
    ...
    closeButton: "!static !order-20 !ml-auto !border-0 !bg-transparent !text-black !shadow-none hover:!bg-neutral-100 [&>svg]:!text-black",
};
/**
...
const RESPONDER_CLASSES = {
    ...
    closeButton: "!static !order-20 !ml-auto !border-0 !bg-transparent !text-white !shadow-none hover:!bg-card-raised [&>svg]:!text-white",
};
const Toaster = ({ variant = "default", ...props }) => {
    const responder = variant === "responder";
    return (<Sonner position="top-right" className="toaster group font-sans" closeButton icons={{
            warning: (<AlertTriangleIcon className="size-4 shrink-0 text-sos" strokeWidth={2.2} aria-hidden/>),
            close: (<XIcon className={cn("size-3.5 shrink-0", responder ? "text-white" : "text-black")} strokeWidth={2.5} aria-hidden/>),
        }} toastOptions={{
