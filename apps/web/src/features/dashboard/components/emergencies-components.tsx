...
import { CheckCircle as CheckCircleIcon, Clock, Crosshair as LocateFixedIcon, MapPin, NavigationArrow, ShieldCheck, Siren, UserCheck } from "@phosphor-icons/react"
import { toast } from "sonner"

import { Button } from "@workspace/ui/components/button"
import { cn } from "@workspace/ui/lib/utils"
import { useAuthSession } from "@/features/auth/auth-session"
import { statusLabel, formatTime, unitLabel } from "@/features/dashboard/components/emergencies-components.utils"
import {
...
export function StatusRail({ status }: { status: EmergencyStatus }) {
  ...
  if (!alert) {
    return (
      <section className="flex min-h-[520px] items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white p-8 text-center">
        <div>
          <ShieldCheck className="mx-auto size-10 text-[#07145f]" />
    ...
    <section className="space-y-4">
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          <InfoTile icon={<MapPin className="size-4" />} label="Location" value={alert.address || alert.barangay} detail={`${Number(alert.latitude).toFixed(5)}, ${Number(alert.longitude).toFixed(5)}`} />
          <InfoTile icon={<UserCheck className="size-4" />} label="Responder" value={assigned ? responderName(assigned) : "Waiting for dispatch"} detail={assigned ? unitLabel[assigned.responder_unit || ""] : "No assignment yet"} />
          <InfoTile icon={<Clock className="size-4" />} label="Latest update" value={latest ? statusLabel[latest.status] : "Submitted"} detail={latest ? formatTime(latest.created_at) : formatTime(alert.created_at)} />
        </div>
      </div>
...
import {
  ResponseTeamSection,
  TeamActionConfirm,
  AssignResponderSection,
  QuickActionsSection,
  SummarySection,
} from "./emergencies-dispatch-panels"
...
export function DispatchPanel({
  ...
  const eligible = useMemo(() => alert ? eligibleUnits[alert.type] : [], [alert])
  const eligibleSet = useMemo(() => new Set(eligible), [eligible])
  const selectedResponderSet = useMemo(() => new Set(selectedResponderIds), [selectedResponderIds])
  const activeAssignments = alert?.assignments.filter(
    (assignment) => !["cancelled", "declined", "resolved"].includes(assignment.status),
  ) ?? []
  const sorted = useMemo(() => {
    if (!alert) return responders
    return responders.filter((responder) => eligibleSet.has(responder.responder_unit || "")).toSorted((a, b) => {
  ...
  return (
    <aside className="space-y-4">
      ...
      <AssignResponderSection
        ...
        alert={alert}
        activeAssignments={activeAssignments}
        selectedResponderSet={selectedResponderSet}
        busyId={busyId}
        busyAction={busyAction}
        eligibleSet={eligibleSet}
        selectedResponderIds={selectedResponderIds}
        unitLabelFn={unitLabel}
        onAssign={(responder) => void assign(responder)}
        onReplace={(responder) => {
          setTeamAction({ kind: "replace", responder })
          setTeamReason("")
        }}
        onToggleSelect={(id) => setSelectedResponderIds((current) => current.includes(id) ? current.filter((i) => i !== id) : [...current, id])}
        onAssignSelected={() => void assignSelected()}
      />
      <QuickActionsSection
        onCallResident={callResident}
        onRefresh={() => void refreshTeam()}
        onEscalate={() => void escalate()}
        onFalseAlarm={() => void markFalseAlarm()}
        busyAction={busyAction}
      />
      <SummarySection alert={alert} responderCount={responders.length} />
    </aside>
  )
}
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]">
        <select
          ...
          aria-label="Select responder unit"
